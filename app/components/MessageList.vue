  ...
  <div class="max-w-3xl w-full mx-auto pt-12">
    <ul class="space-y-2 list-none pl-0" aria-live="polite">
      <li v-for="msg in messages" :key="msg.id" :class="msg.role === 'user' ? 'text-left pr-12' : 'pl-12 '" class="dark:text-gray-200 text-gray-800">
        <span :class="msg.role === 'user' ? 'bg-blue-100 dark:bg-blue-900' : 'bg-gray-100 dark:bg-gray-800'" class="inline-flex px-3 py-2 rounded text-base content">
            ...
            <template v-if="msg.role === 'assistant' && props.fullAssistantMessages && msg.parentId && props.fullAssistantMessages[msg.parentId]">
              <MessageAssistantFull :loading="loading" :content="props.fullAssistantMessages[msg.parentId] || ''" />
            </template>
        </span>
        <span class="sr-only">{{ msg.role }}</span>
      </li>
    </ul>
    <div v-if="streamingThinks && streamingThinks.length" data-section="thinking-stream" class="mt-4 text-left pr-24 " role="status" aria-live="polite" ref="thinksContainer">
      <div
      ...
      <div v-if="props.loading" class="mt-2 text-gray-500">Thinking...</div>
    </div>
  </div>
</template>
...
const props = defineProps<{ messages: Message[]; streamingThinks?: string[]; fullAssistantMessages?: Record<string, string>; loading: boolean }>();
